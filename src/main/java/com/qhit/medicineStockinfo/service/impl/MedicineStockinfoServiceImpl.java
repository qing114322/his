...
public class MedicineStockinfoServiceImpl  implements IMedicineStockinfoService {
    ...
    @Override
    public List<MedicineStockinfo> findall() {
        String sql="SELECT *,CAST(SUM(amt)AS CHAR)AS sumzsl,CAST(SUM(zje)AS CHAR)AS sumzje,CAST((unitprc*1.5)AS CHAR)AS sj \n" +
                "from medicine_stockinfo  \n" +
                "GROUP BY medicinecode_id";
        List<MedicineStockinfo> list = dao.freeFind(sql);
        return list;
    }
